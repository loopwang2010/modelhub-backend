...
// ProviderKeyFlow2API is the registry key for the flow2api upstream.
const ProviderKeyFlow2API adapter.ProviderKey = "flow2api"
...
func (*Adapter) Key() adapter.ProviderKey { return ProviderKeyFlow2API }
...
func (a *Adapter) Submit(ctx context.Context, model adapter.ModelKey, params adapter.Params, idem adapter.IdempotencyKey) (adapter.SubmitResult, error) {
	if idem == "" {
		return nil, fmt.Errorf("flow2api: %w: empty idempotency key", adapter.ErrInvalidParams)
	}
	...
	raw, status, class, err := a.client.postJSONWithKey(ctx, path, spec.AuthKeySelector, idem, body)
	if err != nil {
		return nil, &Error{
			Class:   class,
			Status:  status,
...
func (*Adapter) EstimateCost(model adapter.ModelKey, params adapter.Params) (adapter.CostUSD, error) {
	...
	if !ok {
		return 0, fmt.Errorf("flow2api: %w: unsupported model %q", adapter.ErrInvalidParams, model)
	}
	...
	if cost > adapter.MaxCostUSD {
		return adapter.MaxCostUSD, fmt.Errorf("flow2api: %w: model=%q", adapter.ErrCostCeilingExceeded, model)
	}
	return cost, nil
}
...
func validateParams(model adapter.ModelKey, params adapter.Params) (flowModelSpec, string, []string, error) {
	if params == nil {
		return flowModelSpec{}, "", nil, fmt.Errorf("flow2api: %w: nil params", adapter.ErrInvalidParams)
	}
	spec, ok := modelSpecs[model]
	if !ok {
		return flowModelSpec{}, "", nil, fmt.Errorf("flow2api: %w: unsupported model %q", adapter.ErrInvalidParams, model)
	}
	prompt, err := promptFromParams(params)
	if err != nil {
		return flowModelSpec{}, "", nil, err
	}
	...
	if len(imageURLs) < spec.MinImages {
		return flowModelSpec{}, "", nil, fmt.Errorf("flow2api: %w: model %q requires at least %d image_urls", adapter.ErrInvalidParams, model, spec.MinImages)
	}
	if spec.MaxImages == 0 && len(imageURLs) > 0 {
		return flowModelSpec{}, "", nil, fmt.Errorf("flow2api: %w: model %q does not accept image_urls", adapter.ErrInvalidParams, model)
	}
	if spec.MaxImages > 0 && len(imageURLs) > spec.MaxImages {
		return flowModelSpec{}, "", nil, fmt.Errorf("flow2api: %w: model %q accepts at most %d image_urls", adapter.ErrInvalidParams, model, spec.MaxImages)
	}
...
// ── OpenAI /v1/images/generations request shape ───────────────────────────────
...
// model's manifest spec. Both fields are optional; size defaults to the first
// entry of spec.Sizes, n defaults to 1.
func openAIImageParams(spec flowModelSpec, params adapter.Params) (string, int, error) {
	size := ""
	if raw, ok := params["size"]; ok && raw != nil {
		s, ok := raw.(string)
		if !ok {
			return "", 0, fmt.Errorf("flow2api: %w: size must be a string", adapter.ErrInvalidParams)
		}
	...
	if size != "" && len(spec.Sizes) > 0 {
		...
		if !allowed {
			return "", 0, fmt.Errorf("flow2api: %w: size %q not allowed for model %q", adapter.ErrInvalidParams, size, spec.Key)
		}
	}
	...
	if raw, ok := params["n"]; ok && raw != nil {
		...
		case int:
			n = v
		default:
			return "", 0, fmt.Errorf("flow2api: %w: n must be an integer", adapter.ErrInvalidParams)
		}
	}
	if n < 1 {
		n = 1
	...
	if spec.MaxN > 0 && n > spec.MaxN {
		return "", 0, fmt.Errorf("flow2api: %w: n=%d exceeds max %d for model %q", adapter.ErrInvalidParams, n, spec.MaxN, spec.Key)
	}
	return size, n, nil
}
...
func promptFromParams(params adapter.Params) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("flow2api: %w: prompt is required", adapter.ErrInvalidParams)
	}
	prompt, ok := raw.(string)
	if !ok || strings.TrimSpace(prompt) == "" {
		return "", fmt.Errorf("flow2api: %w: prompt must be a non-empty string", adapter.ErrInvalidParams)
	}
...
func imageURLsFromParams(params adapter.Params) ([]string, error) {
	...
	case []any:
		...
		for _, item := range typed {
			s, ok := item.(string)
			if !ok {
				return nil, fmt.Errorf("flow2api: %w: image_urls must contain only strings", adapter.ErrInvalidParams)
			}
			out = append(out, s)
		}
		return cleanImageURLs(out)
	default:
		return nil, fmt.Errorf("flow2api: %w: image_urls must be an array", adapter.ErrInvalidParams)
	}
}
...
func cleanImageURLs(in []string) ([]string, error) {
	...
	for _, u := range in {
		...
		if u == "" {
			return nil, fmt.Errorf("flow2api: %w: image_urls cannot contain empty strings", adapter.ErrInvalidParams)
		}
		out = append(out, u)
	}
	return out, nil
...
func (e *Error) Error() string {
	return fmt.Sprintf("flow2api: status=%d class=%s: %s", e.Status, e.Class, e.Message)
}
